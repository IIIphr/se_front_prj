...
function Signup() {
  ...
  const [is_loading_page, set_loading_page] = useState(1);
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:4000/api/get-universities",
      {
        method: "POST"
      })
      .then(res => res.json())
                ...
                <Form.Group className='w-100'>
                  <FloatingLabel dir='rtl' className='custom-class' label="دانشگاه">
                    <Form.Select
                      ...
                      defaultValue="دانشگاه">
                        {(data || []).map((record, index) => {
                          return <option key={index}>{record.universityid}</option>
                        })}
                      </Form.Select>
                  </FloatingLabel>
                </Form.Group>
